...
class Image_ImageMagick_Driver extends Image_Driver {
	...
	public function process($image, $actions, $dir, $file, $render = FALSE)
	{
		...
		$this->tmp_image = $dir.'k2img--'.sha1(uniqid(rand(), true).$dir.$file).substr($file, strrpos($file, '.'));
	...
	public function crop($prop)
	{
        ...
        if ($error = exec(escapeshellcmd($this->dir.'convert'.$this->ext).' +repage '.$geometry.' '.$this->cmd_image.' '.$this->cmd_image))
        {
            $this->errors[] = $error;
            return FALSE;
        }
    ...
    {
        if ($error = exec(escapeshellcmd($this->dir.'convert'.$this->ext).' -strip '. escapeshellarg($image) .' '. escapeshellarg($image)))
        {
            $this->errors[] = $error;
            return FALSE;
        }
