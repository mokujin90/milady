...
var ImgManagerDialog = {
    ...
    init : function() {
        ...
        this.populateDirectoriesSelect("directoriesSelect","Выберите каталог для загрузки:"); //Populate folder select infomation
        this.populateDirectoriesSelect("directoriesSelectParent","Выберите родительский каталог:"); //Populate folder select infomation
    ...
    populateDirsAfterUpdate: function(){
        this.populateDirectoriesSelect("directoriesSelect","Выберите каталог для загрузки:");
        this.populateDirectoriesSelect("directoriesSelectParent","Выберите родительский каталог:");
    },
    populateDirectoriesSelect: function(container_id, label){
        $.ajax({
            ...
            success:function(data){
                ...
                select.append($("<option>",{"value":""}).html("Корень"));
    ...
    initImageUpload : function(){
        ...
        $('#uploadify').show();
        if (!tinymce.isIE){
            $('#dropbox').html('<span class=\'message\'><h4>Перетащите сюда файлы для загрузки.</h4></span>').show();
        }
    }
};
...
var Helppers = {
    ...
    getImageHtml: function(fileName){
        ...
        var actionA = $("<a>",{"class":"btn dropdown-toggle","data-toggle":"dropdown","href":"#"}).html("Действие").append($("<span>",{"class":"caret"}));
        var ulAction = $("<ul>",{"class":"dropdown-menu"});
        ulAction.append($("<li>").append($("<a>").html('<i class="icon-play-circle"></i>  Вставить').click(function(){
        ...
        ulAction.append($("<li>").append($("<a>").html('<i class="icon-trash"></i> Удалить').click(function(){
            if(confirm("Удалить? ")){
                $.ajax({
    ...
    getFolderHtml: function(dirName,parentDirName,back){
        ...
        if(back){
            divActions.append($("<p>").append($("<a>", {"class": "btn"}).html("Назад").click(function(){ImgManagerDialog.openDirectory(parentDirName,"../")}) ));
        }else{
            divActions.append( $("<p>").html(dirName) );
            divActions.append( $("<p>").append($("<a>", {"class": "btn btn-info"}).html('<i class="icon-trash icon-white"></i> Удалить').click(function(){
                if(confirm("Удалить каталог со всем его содержимым? ")){
                    var dir = dirName;
                    if(parentDirName){
